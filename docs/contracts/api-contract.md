...
- Docker 镜像必须设置 `PYTHONDONTWRITEBYTECODE=1`，并在依赖安装后清理镜像内 `.pyc` / `__pycache__`，避免 API、worker、db-migrate 等进程在复用或并发导入第三方包时读取损坏的 Python 字节码缓存；本地构建上下文也必须通过 `.dockerignore` 排除 `.venv`、`__pycache__` 和 `*.py[cod]`。
