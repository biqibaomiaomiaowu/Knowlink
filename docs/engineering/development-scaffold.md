...
| Flutter 路由、页面、provider | 第一版主链路已承接；第二版 B站导入区块已接入扫码、预览、创建、轮询、取消、重试和导入后资源刷新 | `client_flutter/lib/app/`、`client_flutter/lib/features/`、`client_flutter/lib/shared/providers/` 已就位；B站导入由 `CourseImportPage` 与 `bilibiliImportProvider` 承接 |
...
| V2 AI 重能力 contract 入口 | 仅轻量类型入口已落地，正式 API / read model / 输出 schema 待冻结 | `server/ai/v2/graph.py`、`streaming.py`、`grading.py` 与 `server/tests/test_ai_v2_contracts.py` 只校验 dataclass / Protocol 边界，不代表图谱、SSE 或主观题判卷运行时已完成 |
