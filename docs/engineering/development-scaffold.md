...
| PostgreSQL / Redis / MinIO / Worker 真实运行时 | 第一版已接通 | 本地运行时已覆盖 SQLAlchemy 持久化仓储、MinIO 上传 / 读取、Redis / Dramatiq worker 与聚合 read model；Docker 后端服务共用 `knowlink-backend:dev` 镜像，镜像构建默认使用中科大 Debian / PyPI 镜像源，并将 apt / Python 第三方依赖层与源码层分离；镜像禁用 Python 字节码缓存，安装后清理 `.pyc` / `__pycache__`，并使用 `pip install --no-compile`；scheduler 当前默认禁用，需 `KNOWLINK_SCHEDULER_ENABLED=true` 显式启用 |
