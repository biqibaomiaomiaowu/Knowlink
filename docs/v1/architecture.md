...
- 文档中的 MVP 承诺已从“接口与模块边界冻结”推进到“首版主链路完成”；具体实现状态以 README 和 [docs/engineering/development-scaffold.md](../engineering/development-scaffold.md) 为准。
- 进度说明：第 2、3、4 周中 `docs/v1/team-division.md` 归杨彩艺 owner 的后端运行时、接口、DB、worker 与联调类任务，首版收口阶段由曹乐代为完成；该说明只记录第一版实际执行过程，不改变 V1 owner 边界。V2 后端分工以 [docs/v2/phase-plan.md](../v2/phase-plan.md) 为准。
- 曹乐 owner 的 Week 1 冻结项与固定联调资料集基线以 [docs/contracts/week1-cao-le-freeze.md](../contracts/week1-cao-le-freeze.md) 和 [docs/v1/demo-assets-baseline.md](./demo-assets-baseline.md) 为准。
- V2 阶段计划、每周任务、负责人分工和验收口径以 [docs/v2/phase-plan.md](../v2/phase-plan.md) 为准；本文中 MVP / 第一版边界不作为 V2 新功能的否定依据。
...
- 社交、排行榜、评论。
- 多人协作。
- 第一版不落通用在线视频平台抓取实现，B 站单视频导入只预留接口 contract；V2 按 `docs/v2/phase-plan.md` 接通 B站真实登录、下载和导入。
- 快应用工程实现。
- 第一版不做全量 WebSocket / SSE 流式输出；V2 按 `docs/v2/phase-plan.md` 做实时流式输出。
- 第一版不做复杂知识图谱；V2 按 `docs/v2/phase-plan.md` 做复杂知识图谱。
- 第一版不做离线下载与离线学习；V2 仍不以离线学习为阶段目标。
- 第一版不做主观题自动判卷；V2 按 `docs/v2/phase-plan.md` 做主观题自动判卷。
...
- 字段级 contract、错误码和示例 payload 统一维护在 [docs/contracts/api-contract.md](../contracts/api-contract.md) 和 [docs/contracts/error-codes.md](../contracts/error-codes.md)。
...
- V1 当前以上接口全部返回 `501`，只冻结 path、字段和错误码。
- V1 只为单个公开视频链接预留 contract，不覆盖番剧、合集、收藏夹和批量解析。
- V2 按 [docs/v2/phase-plan.md](../v2/phase-plan.md) 接通真实扫码登录、下载、合并和课程资源导入，范围包含单视频、多 P、合集、番剧；不做收藏夹、热门列表、大规模批量下载，也不绕过付费、DRM 或地区限制。
...
- `POST /courses/{courseId}/resources/imports/bilibili` 在 V2 接通后返回 `entity.type = bilibili_import_run`；V1 当前阶段统一返回 `501`。
...
1. V1 只冻结单个 B 站公开视频链接的预留接口，不创建真实任务，也不落下载实现。
2. V2 按 `docs/v2/phase-plan.md` 接通扫码登录、元数据预览、下载、ffmpeg 合并、MinIO 上传和课程资源入库。
3. V2 的 B 站导入仍使用 `bilibili_import_run` 作为领域实体，并把下载过程映射到 `async_tasks` 根任务和状态查询。
...
V1.1 是第一版完成后的中间增强候选，不作为当前第二版阶段计划使用；当前 V2 排期、功能范围和负责人分工以 [docs/v2/phase-plan.md](../v2/phase-plan.md) 为准。
...
- B站真实登录、下载和导入，范围包含单视频、多 P、合集、番剧。
- 单用户下的多课程管理；V2 不做多用户、多租户或团队协作。
- 课程库补全与智能课程推荐增强。
- 复杂知识图谱、知识点关系、证据链和可视化。
- 实时流式输出，优先 SSE，WebSocket 作为后续双向场景保留。
- 主观题自动判卷，要求 rubric、证据引用、结构化评分和低置信度兜底。
- Android 环境运行、页面设计优化、复杂版面增强和真实用户测试反馈调研。
- 不把离线缓存、离线学习、独立服务拆分作为 V2 阶段目标；这些只在真实压力点出现后再单独评估。
