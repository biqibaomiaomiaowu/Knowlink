...
适用范围：固定比赛演示路径和走查口径，不包含录屏产物。录屏时按本文执行即可；若录屏环境临时失败，按对应步骤的验收点定位问题。

首版完成状态：

- 截至 2026-05-12，第四周链路已全部跑通，KnowLink 第一版 MVP 已完成。
- 固定完成路径为 `上传 -> 解析 -> 问询 -> 讲义 -> QA -> 测验 -> 复习`。
- 第 2、3、4 周中 `docs/v1/team-division.md` 归杨彩艺 owner 的后端运行时、接口、DB、worker 与联调类任务，首版收口阶段由曹乐代为完成；该说明只记录 V1 实际执行过程，不改变 V1 owner 边界。V2 后端分工以根目录 `docs/v2/phase-plan.md` 为准。

Owner 边界（仅适用于 V1 演示收口）：
...
- `resourceType` 不匹配时以 `docs/v1/demo-assets-baseline.md` 的 MIME 与文件名为准。
...
- 不展示数据库连接串。
- 不展示未接通的非 MVP 功能。
- V1 演示不承诺多课程、多用户、离线缓存或实时流式输出；V2 多课程和实时流式输出以 `docs/v2/phase-plan.md` 为准。
