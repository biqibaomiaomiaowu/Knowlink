...
class HandoutController extends AutoDisposeNotifier<HandoutState> {
  ...
  var _qaRequestId = 0;
  final _autoGeneratedFirstBlockKeys = <String>{};
  final _prefetchedBlockKeys = <String>{};
  ...
  Future<void> load(
    ...
    state = state.copyWith(
      ...
      clearBlockGenerateRequests: true,
      isPolling: false,
  ...
  Future<bool> generateBlock(
    int blockId, {
    required String courseId,
    Duration pollInterval = const Duration(seconds: 2),
    int maxAttempts = 30,
  }) async {
    if (state.isBlockGenerating(blockId)) {
      return false;
    }
    if (state.outline.valueOrNull?.childForBlockId(blockId) == null) {
      return false;
    }
    final effectiveStatus = state.effectiveBlockStatus(blockId);
    if (effectiveStatus == 'ready' || effectiveStatus == 'generating') {
      return false;
    }
    final requestId = _loadRequestId;
    state = state.copyWith(
      blockGenerateRequest: const AsyncLoading(),
      blockGenerateRequestsById: _withBlockGenerateRequest(
        blockId,
        const AsyncLoading(),
      ),
    );
  ...
  Future<void> prefetchNextBlockNearPosition({
    required String? courseId,
    required int positionSec,
    int thresholdSec = 30,
  }) async {
    ...
    final nextChild = state.nextChildNearPosition(
      positionSec,
  ...
  Future<void> prefetchBlockById({
    ...
    if (!accepted) {
      _prefetchedBlockKeys.remove(key);
    }
  }
  ...
  Map<int, AsyncValue<HandoutBlockGenerateResultModel?>>
      _withBlockGenerateRequest(
    int blockId,
    AsyncValue<HandoutBlockGenerateResultModel?> request,
  ) {
    return {
      ...state.blockGenerateRequestsById,
      blockId: request,
    };
  }
  ...
  Future<void> _loadOutlineAndBlocks({
    ...
    try {
      ...
      unawaited(
        _autoGenerateFirstChildIfPending(
          courseId: courseId,
          requestId: requestId,
        ),
      );
  ...
  Future<void> _autoGenerateFirstChildIfPending({
    required String courseId,
    required int requestId,
  }) async {
    if (!_shouldApply(requestId, courseId)) {
    ...
    final firstChild = state.outline.valueOrNull?.children.firstOrNull;
    if (firstChild == null || firstChild.generationStatus != 'pending') {
      return;
    }
    final firstBlock = state.blockForId(firstChild.blockId);
    if (firstBlock != null && firstBlock.status != 'pending') {
      return;
    }
    final key = '$courseId:${firstChild.blockId}';
    if (!_autoGeneratedFirstBlockKeys.add(key)) {
      return;
    }
    final accepted = await generateBlock(
      firstChild.blockId,
    ...
    if (!accepted) {
      _autoGeneratedFirstBlockKeys.remove(key);
    }
  }
  ...
  void _resetForCourseSwitch() {
    _loadRequestId++;
    _invalidateSelectionSideEffects();
    _autoGeneratedFirstBlockKeys.clear();
    _prefetchedBlockKeys.clear();
