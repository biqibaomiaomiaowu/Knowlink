...
enum _QuizPageMode {
  ...
  String get label {
    return switch (this) {
      _QuizPageMode.course => '课程测试',
      _QuizPageMode.lesson => '课时测试',
      _QuizPageMode.detail => '测验详情',
    };
  }

  String get subtitle {
    return switch (this) {
      _QuizPageMode.course => '生成课程范围的客观题测验，提交后查看得分、掌握度变化和下一步复习建议。',
      _QuizPageMode.lesson => '生成当前课时的客观题测验，提交后查看得分、掌握度变化和下一步复习建议。',
      _QuizPageMode.detail => '查看已生成测验的题目、提交结果和复习建议。',
    };
  }

  String? get generateLabel {
    return switch (this) {
      _QuizPageMode.course => '生成课程测试',
      _QuizPageMode.lesson => '生成课时测试',
      _QuizPageMode.detail => null,
    };
  }
}
...
class _QuizPageState extends ConsumerState<QuizPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final mode = _modeForRoute();
    final onGenerate = _buildGenerateAction(mode, courseId);
  ...
  _QuizPageMode _modeForRoute() {
    if (widget.quizId != null) {
      return _QuizPageMode.detail;
    }
    if (widget.lessonId != null) {
      return _QuizPageMode.lesson;
    }
    return _QuizPageMode.course;
  }

  VoidCallback? _buildGenerateAction(_QuizPageMode mode, String? courseId) {
    if (courseId == null) {
      return null;
    }
    switch (mode) {
      case _QuizPageMode.course:
        return () {
          ref.read(quizProvider.notifier).generateAndPoll(courseId);
        };
      case _QuizPageMode.lesson:
        final lessonId = widget.lessonId;
        if (lessonId == null) {
          return null;
        }
        ...
        children: [
          ...
          if (mode.generateLabel != null) ...[
            ...
            FilledButton.icon(
              ...
              label: Text(state.isGenerating ? '生成中' : mode.generateLabel!),
            ),
          ],
        ],
      ),
